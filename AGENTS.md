...
Plus in `app/main.py`: `/healthz` (compose healthcheck), `/robots.txt`, `/sitemap.xml` (index + organisms + companies), and `/{INDEXNOW_KEY}.txt` (IndexNow domain-ownership proof, registered only when the key is configured).
